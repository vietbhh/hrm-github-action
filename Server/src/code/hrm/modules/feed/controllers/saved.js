...
const listPostSaved = async (req, res, next) => {
    try {
        const listPostSaved = await savedMongoModel.find({user_id: req.__user}).select("id -_id").exec()
        ...
        let feeds = await feedMongoModel.find(query).skip((page - 1) * perPage).limit(perPage).exec()

        if (search) {
            const searchTrim = search.trim()
            feeds = feeds.filter(item => item.content.replace(/<[^>]+>/g, "").includes(searchTrim))
        }

        const transformedFeeds = await Promise.all(feeds.map(async (item) => {
