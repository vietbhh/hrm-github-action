// ** React Imports
import { getDefaultFridayLogo, useFormatMessage } from "@apps/utility/common"
...
const EmploymentHistory = (props) => {
  ...
  const renderNumberOfDay = (numberOfDay) => {
    if (numberOfDay === null || numberOfDay === undefined) {
      return ""
    }
      ...
      <CardBody>
        ...
        <PerfectScrollbar>
          <div className="employment-history-list">
            {employeeData.employee_history
              .filter(
                (item) =>
                  item.employment !== null &&
                  item.employment !== undefined &&
                  item.employment !== "" &&
                  !_.isUndefined(item.employment)
              )
              .map((item, index) => {
                const toTime =
