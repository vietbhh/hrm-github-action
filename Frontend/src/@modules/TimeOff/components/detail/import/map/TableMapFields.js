...
const TableMapFields = (props) => {
  ...
  const columns = [
    {
      title: useFormatMessage(
        "modules.time_off_import.text.map_fields_step.field"
      ),
      dataIndex: "field",
      key: "field",
      render: (cellProps) => {
    ...
    {
      title: useFormatMessage(
        "modules.time_off_import.text.map_fields_step.mapping"
      ),
      dataIndex: "mapping",
      key: "mapping",
      align: "center"
    ...
    {
      title: useFormatMessage(
        "modules.time_off_import.text.map_fields_step.upload_file_header"
      ),
  ...
  const dataTable = listFieldImport.map((item, index) => {
    return {
      ...
      upload_file_header: [item.header, item.required]
    };
  });
