...
const SelectViewPermission = (props) => {
  ...
  useEffect(() => {
    if (Object.keys(currentCalendar).length > 0) {
      methods.setValue("view_permission_type", currentOption)
      methods.setValue("office", currentCalendar?.office)
      methods.setValue("department", currentCalendar?.department)
      methods.setValue("specific_employee", currentCalendar?.specific_employee)
    } else {
      methods.setValue("view_permission_type", currentOption)
    }
  }, [currentOption])
