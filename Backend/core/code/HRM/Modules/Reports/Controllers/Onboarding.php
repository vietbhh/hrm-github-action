...
class Onboarding extends ErpController
{
	public function get_onboarding_get()
	{
		...
		foreach ($dataEmployee as $item) {
			$month = date('m/Y', strtotime($item['join_date']));
	...
	public function export_excel_get()
	{
		...
		foreach ($dataEmployee as $item) {
			...
			$sheet->setCellValue("G$i", $item['status']['name_option'] ?? "-");
			$sheet->setCellValue("H$i", date('d/m/Y', strtotime($item['join_date'])));
	...
	private function getDataFilter($filter)
	{
		...
		$dataEmployee = $employeeModel->where("join_date between '" . $filter['date_from'] . "' and '" . $filter['date_to'] . "'")->where('join_date <>', "")->where('join_date <>', null)->where('join_date <>', "0000-00-00")->select("id, employee_code, avatar, full_name, username, join_date, office, department_id, job_title_id, status, employee_type")->orderBy('join_date', 'asc')->asArray()->findAll();
