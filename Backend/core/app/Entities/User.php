...
class User extends \Myth\Auth\Entities\User
{
	...
	public function generateResetHash()
	{
		...
		if (!empty($this->attributes['email'])) {
			$hash = base64_encode($hash . '&' . $this->attributes['email']);
		}
		$this->attributes['reset_hash'] = $hash;
	...
	public function activate(): User
	{
		...
		$this->attributes['account_status'] = 'activated';
	...
	{
		$this->attributes['active'] = 0;
		$this->attributes['account_status'] = 'deactivated';
		return $this;
	}
	...
	public function getRoles()
	{
		...
		if (empty($this->roles)) {
			...
			if (count($groups) > 1) {
				...
				$this->roles = strtolower(array_shift($groups)['name']);
			}
		}
	 ...
	 * @return $this
	 */
	public function setPermissions(array $permissions = null): User
	{
